...
- **可选 AI 精修层**：仅在用户配置 DeepSeek key 并明确点击后联网做保守的段内词汇纠错；原始转写与本地规则优化层始终保留，可随时切回。
...
Vue 2（Options API） · Electron 13 · howler.js · Dexie / IndexedDB v15（含 transcripts / transcriptDict / transcriptAi） · sherpa-onnx-node / SenseVoiceSmall · ffmpeg-static · Vuex · Vue Router（hash 模式）。
...
PodPlayer 0.5.0 的本地 ASR 已在 **Windows x64** 验证。安装包不内置模型；第一次使用前，在设置页「本地转文字稿」手动点击「一键部署模型」，联网下载约 240 MB 的 SenseVoiceSmall 模型文件。部署模型不会自动生成文稿。
...
- **文稿数据**：正文写到 `D:\MyYesPlayerMusic\PodPlayerData\<profile>\transcripts\<episode-sha1>\`，索引与词典 / AI 层写入当前 profile 的 Dexie。删除文稿不会删除音频，删除模型也不会删除既有文稿或音频。
- **DeepSeek 边界**：模型部署与本地转写都不会调用 DeepSeek。AI 精修默认不运行，只有用户自行配置 key 并点击「AI 优化」时，才会把该集文稿发送到所配置的 DeepSeek endpoint。
- **平台边界**：macOS / Linux 客户端仍可构建和使用其它功能，但 0.5.0 不宣称本地 ASR 已在这些平台验证；设置页会把当前平台明确降级为不支持。
...
- **自动备份**：导出为全保真 JSON + 订阅 OPML，落在当前 profile 的 `backups\` 下，保留最近 3 份；为瘦身不含 `episodes` 全表（可由 OPML 重订阅重抓）。备份包含文稿索引、词典和 AI 精修层，但文稿正文仍保留在 profile 的 `transcripts\`。内置「空库不备份」安全阀，避免清空事故后用空数据覆盖历史好备份。
...
> 0.5.0 的本地 ASR 发布验证范围是 Windows x64。其它平台构建不应因 ASR 缺失而阻止应用启动，但不提供已验证的本地转录承诺。
