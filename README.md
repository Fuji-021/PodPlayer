...
PodPlayer 0.7.0 的本地 ASR 已在 **Windows x64** 验证。安装包不内置模型；第一次使用前，在设置页「本地转文字稿」手动点击「一键部署模型」，联网下载约 240 MB 的 SenseVoiceSmall 模型文件。部署模型不会自动生成文稿。
...
- **平台边界**：macOS / Linux 客户端仍可构建和使用其它功能，但 0.7.0 不宣称本地 ASR 已在这些平台验证；设置页会把当前平台明确降级为不支持。
...
> 0.7.0 的本地 ASR 发布验证范围是 Windows x64。其它平台构建不应因 ASR 缺失而阻止应用启动，但不提供已验证的本地转录承诺。Windows 安装包当前未做 Authenticode 签名，首次运行可能显示 SmartScreen 提示。
