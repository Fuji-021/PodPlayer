...
- **真实收听统计**：按「天 × 节目」聚合收听时长，统计页可视化（最近一周 / 全部两种范围）+ 非线性排行重排动画；可独立启用节目封面色彩纹理，设置页可导出为 CSV / JSON。
...
- **可选联网 AI 层**：设置页提供 DeepSeek、OpenAI、Gemini 兼容接口、OpenRouter、本地服务和自定义 OpenAI Chat Completions 服务的预设；用户“保存并测试”成功后才可主动生成本集总结或精修文字稿。密钥受 Windows DPAPI 保护，音频不会上传；DeepSeek 已实机验证，其它服务仅为协议兼容支持。
...
- **本地数据自动备份**：订阅 / 进度 / 统计 / 收藏 / 下载记录定期落盘（含订阅 OPML）；完整恢复在预检、恢复前快照和单事务回灌后执行，历史合并不会覆盖文稿资产。
...
PodPlayer 0.8.0 的本地 ASR 已在 **Windows x64** 验证。安装包不内置模型；第一次使用前，在设置页「本地转文字稿」手动点击「一键部署模型」，联网下载约 240 MB 的 SenseVoiceSmall 模型文件。部署模型不会自动生成文稿。
...
- **音频前置条件**：ASR 只处理本地音频。对未永久下载单集明确点击转录或“生成文字稿并总结”时，会在当前任务内临时准备音频，再加入单任务转录队列；完成后只保留文字稿和总结，不保留临时音频，也不会进入下载列表。已存在的永久下载会直接复用。
...
- **联网 AI 边界**：模型部署与本地转写都不会调用联网 AI。精修和本集总结默认不运行，只有用户“保存并测试”配置成功并明确点击后，才会把该集文字稿发送到所配置的服务；音频不会上传。远程 HTTP 服务会被拒绝，仅允许 HTTPS 或本机回环 HTTP。DeepSeek 已实机验证，其他预设及自定义服务只承诺 OpenAI Chat Completions 协议兼容。
- **平台边界**：macOS / Linux 客户端仍可构建和使用其它功能，但 0.8.0 不宣称本地 ASR 已在这些平台验证；设置页会把当前平台明确降级为不支持。
...
- **自动备份**：导出为全保真 JSON + 订阅 OPML，落在当前 profile 的 `backups\` 下，保留最近 3 份；为瘦身不含 `episodes` 全表（可由 OPML 重订阅重抓）。备份包含文稿索引、词典、AI 精修层和本集总结，但文稿正文仍保留在 profile 的 `transcripts\`。完整恢复会先预检、保留独立恢复前快照，并在单一事务中回灌；失败时只报告真实的回滚状态。内置「空库不备份」安全阀，避免清空事故后用空数据覆盖历史好备份。
- **合并恢复历史**：该入口只恢复收藏、进度、收听统计、每日统计和下载记录；不会覆盖订阅、单集、文稿、词典、AI 精修或本集总结。
...
> 0.8.0 的本地 ASR 发布验证范围是 Windows x64。其它平台构建不应因 ASR 缺失而阻止应用启动，但不提供已验证的本地转录承诺。Windows 安装包当前未做 Authenticode 签名，首次运行可能显示 SmartScreen 提示。
...
- **为什么未下载单集也能生成文字稿？** 本地 ASR 需要本地音频。明确点击后，PodPlayer 仅为本次转录临时准备音频；暂停或可重试失败会在有限时间内保留以便续转，成功、明确取消或永久失败后会清理，不会成为永久下载。
