...
环境变量（默认值见下方 E(...)）：
  ...
  TRIM_ENABLED=0  # [治本] 每档主动裁到 KEEP_PER_PODCAST 集(ABS maxEpisodesToKeep 不回溯历史→只增不减)。
                  #   默认 0=DRY-RUN 只在日志报"会删多少"不真删；观察满意后设 1 真裁。总量自然远低于 CAP。
...
def trim_per_podcast():
    ...
    #   自然远低于 CAP、evict 几乎不触发。autoDownload 只追"新发布集"，不会重下已删老集。
    #   TRIM=0(默认) → DRY-RUN：只统计"会删多少"、不真删；观察日志满意后设 TRIM_ENABLED=1 真裁。
    if not TOK:return
    try:
        _,it=api("/api/libraries/%s/items?limit=300"%LIB)
    except Exception as e:
        log("trim query fail %s"%e);return
    arr=(it.get("results") or it.get("items") or []) if isinstance(it,dict) else []
    would=0;deleted=0;shows=0
    for x in arr:
        iid=x.get("id")
        if not iid:continue
        try:
            ...
            continue
        es=(j.get("media",{}) or {}).get("episodes",[]) if isinstance(j,dict) else []
        if len(es)<=KEEP:continue
        shows+=1
        es2=sorted(es,key=lambda e:(e.get("publishedAt") or e.get("addedAt") or 0))
        ndel=len(es2)-KEEP
        would+=ndel
    ...
    if TRIM:
        log("trim: deleted %d old episodes across %d shows (keep %d/podcast)"%(deleted,shows,KEEP))
    else:
        log("trim DRY-RUN: would delete %d episodes across %d shows to keep %d/podcast (set TRIM_ENABLED=1 to apply)"%(would,shows,KEEP))
log("watchdog up: every %ds, ABS=%s container=%s stall=%.0fmin cap=%.0fGB evict=%s trim=%s"%(ITV,ABS,CON,STALL,CAP,EVICT,TRIM))
while True:
    try:
        check_download()
    except Exception as e:
