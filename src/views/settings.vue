      ...
      <section v-if="isElectron" class="settings-feature-block">
        <div class="item settings-feature-item">
          <div class="left">
            ...
            <div class="description">
              部署模型后，可在有可用音频地址的单集详情页手动生成文字稿；未永久下载时会临时准备音频，文字稿保存后自动清理临时媒体。生成后可在沉浸页点击封面查看。
            </div>
          </div>
          <div class="right settings-feature-actions">
            <span
  ...
  computed: {
    ...
    asrDownloadTitle() {
      ...
      if (!this.asrModel.platformSupported)
        return 'PodPlayer 0.8.0 的本地转文字稿仅验证 Windows x64';
      return '当前无法使用远程部署，请稍后重试或选择本地模型目录';
    },
    asrModelStatusText() {
      if (this.asrModel.installing) return '正在部署模型';
      if (!this.asrModel.platformSupported) {
        return '当前平台暂不支持。本地转文字稿在 0.8.0 仅验证 Windows x64。';
      }
