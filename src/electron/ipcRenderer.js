...
export function ipcRenderer(vueInstance) {
  ...
  // Main-process shortcut registration is authoritative. Background state
  // updates only paint the settings rows; user actions decide whether to toast.
  ipcRenderer.on('shortcut:state', (event, shortcutState) => {
    store.commit('applyShortcutRuntimeState', shortcutState);
  });
